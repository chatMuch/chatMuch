...
router.get('/:model/:id', bearerAuth, permissions('read'), handleGetSome); // sends back all customers associated with a sales person
...
async function handleDelete(req, res) {
  ...
  let recordToDelete = await req.model
  .findOne({where: { id: id }});

  recordToDelete.destroy();

  res.status(202).json(recordToDelete);
}
